...
const char receive_loop[] =
    R"(function receive(id, message);
      local buffer = {};
      for i=1,1500 do
        if((i-1)%(1500/20)==0) then
          buffer[i]=2
        else
          buffer[i]=1
        end
      end
      send(id, (id+1)%16, buffer);
      end)";
#else
const char receive_loop[] =
    R"(function receive(id, message);
...
class LuaActor {
 public:
  LuaActor(uint64_t id, Router* router) : id(id), router(router) {
    ...
    if (luaL_dostring(lua_state, receive_loop)) {
  ...
  ~LuaActor() { router->deregister_actor(id); }
  ...
  void receive(uint64_t sender, char* message) {
    ...
    lua_pushlstring(lua_state, message, 1500);
...
#if TOUCH_BYTEWISE
      if (lua_tointeger(state, -2) < 1506) {
        *(m.buffer() + lua_tointeger(state, -2) - 1) =
            48 + lua_tointeger(state, -1);
      }
#else
      if (lua_tointeger(state, -2) < 376) {
