...
ssh -R $TESTBED_SERVER_PORT:$TESTBED_SERVER_HOST:$TESTBED_SERVER_PORT cm-pi1 << EOF
  bash --login -c "echo 'called' && python3 $ROOT_DIR/tools/peer_announcer.py && sleep 30 && python3 $ROOT_DIR/tools/peer_announcer.py && sleep 30 && python3 $ROOT_DIR/tools/actorctl.py \$(python3 $ROOT_DIR/tools/testbed_ip_for.py node_1) 1337 $ROOT_DIR/assets/evaluation_multi_esp32_hop_count.yml"
EOF