...
ssh -R $TESTBED_SERVER_PORT:$TESTBED_SERVER_HOST:$TESTBED_SERVER_PORT cm-pi1 << EOF
  bash --login -c "python3 $ROOT_DIR/evaluation_tools/peer_announcer_linear.py && sleep 60 && python3 $ROOT_DIR/evaluation_tools/peer_announcer_linear.py && sleep 60 && python3 $ROOT_DIR/tools/actorctl.py \$(python3 $ROOT_DIR/evaluation_tools/testbed_ip_for.py node_1) 1337 -f $ROOT_DIR/assets/evaluation_multi_esp32_element_size_$1_hop.yml"
EOF