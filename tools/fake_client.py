...
parser.add_argument("sender_node_id")
parser.add_argument("host")
parser.add_argument("port", type=int)
parser.add_argument("server_node_id")
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    sub_message = {}
    sub_message["sender_node_id"] = arguments.sender_node_id
    sub_message["sender_actor_type"] = "test_actor"
    sub_message["sender_instance_id"] = "1"
    sub_message["type"] = "subscription_update"
    sub_message["subscription_node_id"] = arguments.sender_node_id
    sub_msg = msgpack.packb(sub_message)
    s.send(struct.pack("!i", len(sub_msg)))
    s.send(sub_msg)

    spawn_message = {}
    spawn_message["sender_node_id"] = arguments.sender_node_id
    spawn_message["sender_actor_type"] = "test_actor"
    spawn_message["spawn_node_id"] = arguments.server_node_id
    spawn_message["spawn_actor_type"] = "remotely_spawned_actor"
    spawn_message["spawn_instance_id"] = arguments.sender_node_id
    spawn_message["spawn_code"] = f"""
        function receive(message)
            ...
            if(message["type"] == "init" or message["type"] == "periodic_timer") then
                send({{node_id="{arguments.sender_node_id}", actor_type="test_actor", instance_id="1", message="ping"}});
            ...
            if(message["type"] == "exit") then
                send({{node_id="{arguments.sender_node_id}", actor_type="test_actor", instance_id="1", message="goodbye"}});
            end 
        end"""
    spawn_message["node_id"] = arguments.server_node_id
    ...
    while True:
        ...
        out_message = {}
        out_message["sender_node_id"] = arguments.sender_node_id
