...
ssh -R 192.168.50.3:1337:192.168.50.3:1337 -R $TESTBED_SERVER_PORT:$TESTBED_SERVER_HOST:$TESTBED_SERVER_PORT cm-pi1 << EOF
  bash --login -c "echo 'called' && python3 $ROOT_DIR/tools/peer_announcer.py && echo "done" && sleep 30 && python3 $ROOT_DIR/tools/peer_announcer.py && sleep 30 && python3 $ROOT_DIR/tools/actorctl.py \$(python3 $ROOT_DIR/tools/testbed_ip_for.py node_edge) 1337 $ROOT_DIR/assets/data_processing_$1.yml"
EOF